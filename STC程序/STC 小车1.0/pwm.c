...
void PWM_Out(unsigned char ch, unsigned char duty)   //duty 0~100
{
	if(ch==0)
	{
		CCAP0H=CCAP0L=256-256*duty/100;  //设置占空比
	}
	if(ch==1)
	{
		CCAP1H=CCAP1L=256-256*duty/100;  //设置占空比
	}
	if(ch==2)
	{
		CCAP2H=CCAP2L=256-256*duty/100;  //设置占空比
	}
}